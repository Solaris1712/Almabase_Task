...
def find_duplicates(profiles:List,profile_fields:List[str]):
    ...
    # This is where both scripts differ, this script just goes ahead and does field checking here itself.
    # Even though we can see 3 nested for loops, the time complexity is the same as we are not looping over the entire profile or duplicates_table everytime.
    # The inner ones are just a handful of loops, orders of magnitudes lower than profiles.
    # The performance should be the same, just provided both in case these differ in multi-threaded environments.
    for ele in profiles:
            ...
            for index in duplicates:
                ...
                for field in profile_fields:
                    if field != 'email first_name last_name':
                        first = profiles[cur_index].get(field,None)
                        second = profiles[index].get(field,None)
                        if first and second:
                            if first == second:
                                duplicates_table[(cur_index,index)]['match_score'] += 1
                                duplicates_table[(cur_index,index)]['matching_attributes'].append(field)
                            else:
                                duplicates_table[(cur_index,index)]['match_score'] -= 1
                                duplicates_table[(cur_index,index)]['non_matching_attributes'].append(field)
                        else:
                            duplicates_table[(cur_index,index)]['ignored_attributes'].append(field)
                    else:
                        continue
